 ...
 */
struct vfs_superblock_t *vfs_mount_fs(char *name, void *DPTE, uint8_t DPT_type, void *buf, int8_t part_num)
{
    ...
    for (p = &vfs_fs; p; p = p->next)
    {
        ...
        {
            return p->read_superblock(DPTE, DPT_type, buf, part_num);
        }
    }
...
struct vfs_dir_entry_t *vfs_path_walk(const char *path, uint64_t flags)
{
    ...
    while (true)
    {
        ...
        // 为目录项的名称分配内存
        dentry->name = (char *)kalloc(tmp_path_len + 1);
        // 貌似这里不需要memset，因为空间会被覆盖
        // memset(dentry->name, 0, tmp_path_len+1);

        memcpy((void *)tmp_path, dentry->name, tmp_path_len);
...
uint64_t sys_mkdir(struct pt_regs *regs)
{
    ...
    char *buf = (char *)kalloc(last_slash + 1);
    ...
    struct vfs_dir_entry_t *subdir_dentry = (struct vfs_dir_entry_t *)kalloc(sizeof(struct vfs_dir_entry_t));
    ...
    subdir_dentry->name = (char *)kalloc(subdir_dentry->name_length + 1);
...
void init_fs()
{
    root_dev = device_find(DEV_DISK, 0)->dev;
}
