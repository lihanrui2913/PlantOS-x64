 ...
 */
struct vfs_superblock_t *fat32_register_partition(uint8_t part_num)
{
    ...
    // 读取文件系统的boot扇区
    device_read(root_dev, buf, 512, DPT->DPTE[part_num].starting_LBA, 0);

    // 挂载文件系统到vfs
    return vfs_mount_fs("FAT32", (void *)(&DPT->DPTE[part_num]), VFS_DPT_MBR, buf, part_num);
}
...
struct vfs_dir_entry_t *fat32_lookup(struct vfs_index_node_t *parent_inode, struct vfs_dir_entry_t *dest_dentry)
{
    ...
    uint8_t *buf = kalloc(fsbi->bytes_per_clus);
    ...
    while (true)
    {
        ...
        // 读取父目录项的起始簇数据
        device_read(root_dev, buf, fsbi->sec_per_clus * 512, sector, 0);
...
find_lookup_success:; // 找到目标dentry
    struct vfs_index_node_t *p = (struct vfs_index_node_t *)kalloc(sizeof(struct vfs_index_node_t));
    ...
    p->private_inode_info = (void *)kalloc(sizeof(fat32_inode_info_t));
 ...
 */
struct vfs_superblock_t *fat32_read_superblock(void *DPTE, uint8_t DPT_type, void *buf, int8_t part_num)
{
    ...
    // 分配超级块的空间
    struct vfs_superblock_t *sb_ptr = (struct vfs_superblock_t *)kalloc(sizeof(struct vfs_superblock_t));
    memset(sb_ptr, 0, sizeof(struct vfs_superblock_t));

    sb_ptr->sb_ops = &fat32_sb_ops;
    sb_ptr->private_sb_info = kalloc(sizeof(fat32_sb_info_t));
    ...
    device_read(root_dev, &fsbi->fsinfo, 512, MBR_DPTE->starting_LBA + fbs->BPB_FSInfo, 0);
    color_printk(BLUE, BLACK, "FAT32 FSInfo\n\tFSI_LeadSig:%#018lx\n\tFSI_StrucSig:%#018lx\n\tFSI_Free_Count:%#018lx\n", fsbi->fsinfo.FSI_LeadSig, fsbi->fsinfo.FSI_StrucSig, fsbi->fsinfo.FSI_Free_Count);

    // 初始化超级块的dir entry
    sb_ptr->root = (struct vfs_dir_entry_t *)kalloc(sizeof(struct vfs_dir_entry_t));
    ...
    // 分配2个字节的name
    sb_ptr->root->name = (char *)(kalloc(2));
    sb_ptr->root->name[0] = '/';
    sb_ptr->root->name_length = 1;

    // 为root目录项分配index node
    sb_ptr->root->dir_inode = (struct vfs_index_node_t *)kalloc(sizeof(struct vfs_index_node_t));
    ...
    // 初始化inode信息
    sb_ptr->root->dir_inode->private_inode_info = kalloc(sizeof(struct fat32_inode_info_t));
...
void fat32_write_inode(struct vfs_index_node_t *inode)
{
    ...
    struct fat32_Directory_t *buf = (struct fat32_Directory_t *)kalloc(fsbi->bytes_per_clus);
    memset(buf, 0, fsbi->bytes_per_clus);
    device_read(root_dev, buf, fsbi->sec_per_clus, fLBA, 0);
    ...
    // 将dir entry写回磁盘
    device_write(root_dev, buf, fsbi->sec_per_clus, fLBA, 0);
...
long fat32_read(struct vfs_file_t *file_ptr, char *buf, int64_t count, long *position)
{
    ...
    void *tmp_buffer = kalloc(fsbi->bytes_per_clus);
    ...
    do
    {
        ...
        // 读取一个簇的数据
        device_read(root_dev, tmp_buffer, fsbi->sec_per_clus * 512, sector, 0);
...
long fat32_write(struct vfs_file_t *file_ptr, char *buf, int64_t count, long *position)
{
    ...
    do
    {
        ...
        // 写入数据到对应的簇
        device_write(root_dev, tmp_buffer, fsbi->sec_per_clus * 512, sector, 0);
...
long fat32_create(struct vfs_index_node_t *parent_inode, struct vfs_dir_entry_t *dest_dEntry, int mode)
{
    ...
    struct vfs_index_node_t *inode = (struct vfs_index_node_t *)kalloc(sizeof(struct vfs_index_node_t));
    memset((void *)inode, 0, sizeof(struct vfs_index_node_t));
    dest_dEntry->dir_inode = inode;
    dest_dEntry->dir_ops = &fat32_dEntry_ops;

    struct fat32_inode_info_t *finode = (struct fat32_inode_info_t *)kalloc(sizeof(struct fat32_inode_info_t));
    ...
    device_write(root_dev, (void *)tmp_dentry_clus_buf_addr, fsbi->sec_per_clus * 512, tmp_dentry_sector, 0);
...
int64_t fat32_mkdir(struct vfs_index_node_t *parent_inode, struct vfs_dir_entry_t *dEntry, int mode)
{
    ...
    inode->private_inode_info = (fat32_inode_info_t *)kalloc(sizeof(fat32_inode_info_t));
    ...
    // ====== 初始化新的文件夹的目录项 =====
    {
        ...
        void *buf = kalloc(fsbi->bytes_per_clus);
        ...
        device_write(root_dev, buf, fsbi->sec_per_clus * 512, sector, 0);
    }
...
int64_t fat32_readdir(struct vfs_file_t *file_ptr, void *dirent, vfs_filldir_t filler)
{
    ...
    unsigned char *buf = (unsigned char *)kalloc(fsbi->bytes_per_clus);
    uint32_t cluster = finode->first_clus;
    ...
    while (cluster <= 0x0ffffff7) // cluster在循环末尾更新（如果当前簇已经没有短目录项的话）
    {
        ...
        for (int i = file_ptr->position % fsbi->bytes_per_clus; i < fsbi->bytes_per_clus; i += 32, file_ptr->position += 32, ++dentry)
        {
            ...
            if (long_dentry->LDIR_Attr == ATTR_LONG_NAME && long_dentry->LDIR_Ord != 0xe5 && long_dentry->LDIR_Ord != 0x00 && long_dentry->LDIR_Ord != 0x05)
            {
                ...
                // 为目录名分配空间
                dir_name = (char *)kalloc(count_long_dentry * 26 + 1);
...
void init_fat32()
{
    ...
    // 挂载根文件系统
    vfs_root_sb = fat32_register_partition(0);
    ksuccess("FAT32 initialized.");
}
