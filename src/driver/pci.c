...
static uint32_t count_device_list = 0;
...
uint32_t pci_read_config(uint8_t bus, uint8_t slot, uint8_t func, uint8_t offset)
{
    ...
    // 读取返回的数据
    uint32_t ret = (uint32_t)(io_in32(PORT_PCI_CONFIG_DATA));
 ...
 * @return uint32_t 返回码
 */
uint32_t pci_write_config(uint8_t bus, uint8_t slot, uint8_t func, uint8_t offset, uint32_t data)
{
    uint32_t lbus = (uint32_t)bus;
    uint32_t lslot = (uint32_t)slot;
    uint32_t lfunc = ((uint32_t)func) & 7;
 ...
 * @param func 功能号
 */
static void pci_read_general_device_header(struct pci_device_structure_general_device_t *header, uint8_t bus, uint8_t slot, uint8_t func)
{
    uint32_t tmp32;
 ...
 * @param func 功能号
 */
static void pci_read_pci_to_pci_bridge_header(struct pci_device_structure_pci_to_pci_bridge_t *header, uint8_t bus, uint8_t slot, uint8_t func)
{
    uint32_t tmp32;
    header->BAR0 = pci_read_config(bus, slot, func, 0x10);
    header->BAR1 = pci_read_config(bus, slot, func, 0x14);
 ...
 * @param func 功能号
 */
static void pci_read_pci_to_cardbus_bridge_header(struct pci_device_structure_pci_to_cardbus_bridge_t *header, uint8_t bus, uint8_t slot, uint8_t func)
{
    uint32_t tmp32;
 ...
 * @return 返回的header
 */
void *pci_read_header(int *type, uint8_t bus, uint8_t slot, uint8_t func, bool add_to_list)
{
    struct pci_device_structure_header_t *common_header = (struct pci_device_structure_header_t *)kalloc(127);
...
void pci_get_device_structure(uint8_t class_code, uint8_t sub_class, struct pci_device_structure_header_t *res[], uint32_t *count_res)
{
    ...
    *count_res = 0;
    ...
    for (uint32_t i = 0; i < count_device_list; ++i)
    {
        ...
        {
            kdebug("[%d]  class_code=%d, sub_class=%d, progIF=%d", i, ptr->Class_code, ptr->SubClass, ptr->ProgIF, ((struct pci_device_structure_general_device_t *)ptr)->BAR5);
