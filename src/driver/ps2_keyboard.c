 ...
 * @param regs 寄存器信息
 */
void ps2_keyboard_handler(uint64_t irq_num, uint64_t buf_vaddr, struct pt_regs *regs)
{
...
void init_ps2keyboard()
{
    ...
    // 注册中断处理程序
    irq_register(PS2_KEYBOARD_INTR_VECTOR, &entry, &ps2_keyboard_handler, (uint64_t)&kb_buf, &ps2_keyboard_intr_controller, "ps/2 keyboard");
