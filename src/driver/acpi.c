                   ...
                   void *_data)
{
    ...
    if (acpi_use_xsdt)
    {
        ...
        {
            vmm_mmap((uint64_t)get_cr3(), true, ACPI_XSDT_DESCRIPTION_HEDERS_BASE + PAGE_2M_SIZE * i, (*(ent + i)) & PAGE_2M_MASK, PAGE_2M_SIZE, PAGE_PRESENT | PAGE_R_W, false, true);
...
void acpi_init()
{
    ...
    vmm_mmap((uint64_t)get_cr3(), true, rsdp_address, (uint64_t)rsdp_request.response->address, PAGE_2M_SIZE, PAGE_PRESENT | PAGE_R_W | PAGE_PCD | PAGE_PWT, false, true);
    ...
    if (rsdpv2->XsdtAddress != 0x00UL)
    {
        ...
        // vmm_mmap((uint64_t)get_cr3(), true, ACPI_XSDT_VIRT_ADDR_BASE, xsdt_phys_base, PAGE_2M_SIZE, PAGE_PRESENT | PAGE_R_W | PAGE_PCD | PAGE_PWT, false, true);
        // kdebug("XSDT mapped!");
        ...
        // vmm_mmap((uint64_t)get_cr3(), true, ACPI_XSDT_VIRT_ADDR_BASE, xsdt_phys_base, xsdt->header.Length + PAGE_2M_SIZE, PAGE_PRESENT | PAGE_R_W | PAGE_PCD | PAGE_PWT, false, true);
        // // 映射所有的Entry的物理地址
        // uint64_t *ent = &(xsdt->Entry);
        ...
        //     // 映射RSDT ENTRY的物理地址
        //     vmm_mmap((uint64_t)get_cr3(), true, ACPI_XSDT_DESCRIPTION_HEDERS_BASE + PAGE_2M_SIZE * j, (*(ent + j)), PAGE_2M_SIZE, PAGE_PRESENT | PAGE_R_W | PAGE_PCD | PAGE_PWT, false, true);
        // }
        ...
        vmm_mmap((uint64_t)get_cr3(), true, ACPI_RSDT_VIRT_ADDR_BASE, rsdt_phys_base, PAGE_2M_SIZE, PAGE_PRESENT | PAGE_R_W | PAGE_PCD | PAGE_PWT, false, true);
        ...
        vmm_mmap((uint64_t)get_cr3(), true, ACPI_RSDT_VIRT_ADDR_BASE, rsdt_phys_base, rsdt->header.Length + PAGE_2M_SIZE, PAGE_PRESENT | PAGE_R_W | PAGE_PCD | PAGE_PWT, false, true);
        // 映射所有的Entry的物理地址
        ...
        // 映射RSDT ENTRY的物理地址
        vmm_mmap((uint64_t)get_cr3(), true, ACPI_DESCRIPTION_HEDERS_BASE, acpi_RSDT_entry_phys_base, PAGE_2M_SIZE, PAGE_PRESENT | PAGE_R_W | PAGE_PCD | PAGE_PWT, false, true);
    }
