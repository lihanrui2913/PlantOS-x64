 ...
 */
uint32_t kfifo_in(struct kfifo_t *fifo, void *from, uint32_t size)
{
    // 判断空间是否够
    if (fifo->size + size > fifo->total_size)
        return 0;
    if (from == NULL)
        return 0;
...
{
    if (to == NULL) // 判断目标地址是否为空
        return 0;
    if (size > fifo->size) // 判断队列中是否有这么多数据
        return 0;
